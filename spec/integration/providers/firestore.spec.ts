...
describe('providers/firestore', () => {
  ...
  it('clears database with clearFirestoreData', async () => {
    ...
    await Promise.all([
      db.collection('test').doc('doc1').set({}),
      db.collection('test').doc('doc1').collection('test').doc('doc2').set({}),
    ]);

    await test.firestore.clearFirestoreData({ projectId: 'not-a-project' });

    const docs = await Promise.all([
      db.collection('test').doc('doc1').get(),
      db.collection('test').doc('doc1').collection('test').doc('doc2').get(),
    ]);
